...
public abstract class BlockModCrop extends BlockBush implements IGrowable {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return ((Integer)state.getValue(getAge())).intValue() == getStages() ? this.getCrop() : this.getSeed();
	}

	@Override
	public boolean canGrow(World worldIn, BlockPos pos, IBlockState state, boolean isClient) {
		return ((Integer)state.getValue(getAge())).intValue() < getStages();
	}
	...
	public int getMetaFromState(IBlockState state) {
		return ((Integer)state.getValue(getAge())).intValue();
	}

	@Override
	protected BlockState createBlockState() {
		return new BlockState(this, new IProperty[] {getAge()});
	}
