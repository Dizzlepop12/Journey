...
public class ModTeleporter extends Teleporter {
	...
	public void placeInPortal(Entity entity, float par8) {
		if (this.worldServerInstance.provider.getDimensionId() != Config.corba) {
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? frameBlock.getDefaultState() : Blocks.air.getDefaultState());
					}
				}
			}
