...
import net.journey.dimension.golden.gen.WorldGenGoldenStalks;
...
public class ChunkProviderGoldenGrains implements IChunkProvider {
					...
					for(int l2 = 0; l2 < 8; ++l2) {
						...
						for(int i3 = 0; i3 < 4; ++i3) {
							...
							for (int j3 = 0; j3 < 4; ++j3) {
								if ((d15 += d16) > 0.0D) {
									p_180518_3_.setBlockState(k * 4 + i3, k2 * 8 + l2, j1 * 4 + j3, JourneyBlocks.goldenStone.getDefaultState());
								}
							}
	...
	public final void generateBiomeTerrain(Random r, ChunkPrimer c, int x, int z, double s) {
		boolean flag = true;
		IBlockState iblockstate = JourneyBlocks.goldenGrass.getDefaultState();
		IBlockState iblockstate1 = JourneyBlocks.goldenStone.getDefaultState();
		int k = -1;
		...
		for(int k1 = 255; k1 >= 0; --k1) {
			if(k1 <= 1) {
				...
				else if(iblockstate2.getBlock() == JourneyBlocks.goldenStone) {
					if(k == -1) {
						if(l <= 0) {
							iblockstate = null;
							...
							iblockstate = JourneyBlocks.goldenGrass.getDefaultState();
							iblockstate1 = JourneyBlocks.goldenStone.getDefaultState();
						}

						if(k1 < 63 && (iblockstate == null || iblockstate.getBlock().getMaterial() == Material.air))
							iblockstate = JourneyBlocks.goldenStone.getDefaultState();
						k = l;
