...
public class ModelPurplian extends ModelBase {
	...
	public void setRotationAngles(float f, float f2, float f3, float f4, float f5, float f7, Entity e) {
		...
		for(i = 0; i < 1; i++) {
			this.sticks[i].rotationPointY = -5.0F + MathHelper.cos(((float)(i * 3) + f3) * 0.25F);
		...
		for(i = 1; i < 2; i++) {
			this.sticks[i].rotationPointY = 2.0F + MathHelper.cos(((float)(i * 2) + f3) * 0.25F);
		...
		for(i = 2; i < 4; i++) {
			this.sticks[i].rotationPointY = 11.0F + MathHelper.cos(((float)i * 1.5F + f3) * 0.5F);
