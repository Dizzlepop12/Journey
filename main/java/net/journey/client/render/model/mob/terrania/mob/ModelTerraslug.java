...
public class ModelTerraslug extends ModelBase
{
    ...
    public ModelTerraslug()
    {
        ...
        for (int i = 0; i < this.terraslugBodyParts.length; ++i)
        {
            ...
            this.terraslugBodyParts[i].addBox
            		((float)terraslugBoxLength[i][0] * -0.5F, 0.0F, 
            		(float)terraslugBoxLength[i][2] * -0.5F, 
            		terraslugBoxLength[i][0], 
            		terraslugBoxLength[i][1], 
            		terraslugBoxLength[i][2]);
            this.terraslugBodyParts[i].setRotationPoint(0.0F, (float)(24 - terraslugBoxLength[i][1]), f);
            this.field_78170_c[i] = f;

            if (i < this.terraslugBodyParts.length - 1)
            {
                f += (float)(terraslugBoxLength[i][2] + terraslugBoxLength[i + 1][2]) * 0.5F;
            }
        }

        this.terraslugWings = new ModelRenderer[3];
        this.terraslugWings[0] = new ModelRenderer(this, 20, 0);
        this.terraslugWings[0].addBox(-5.0F, 0.0F, (float)terraslugBoxLength[2][2] * -0.5F, 10, 8, terraslugBoxLength[2][2]);
        this.terraslugWings[0].setRotationPoint(0.0F, 16.0F, this.field_78170_c[2]);
        this.terraslugWings[1] = new ModelRenderer(this, 20, 11);
        this.terraslugWings[1].addBox(-3.0F, 0.0F, (float)terraslugBoxLength[4][2] * -0.5F, 6, 4, terraslugBoxLength[4][2]);
        this.terraslugWings[1].setRotationPoint(0.0F, 20.0F, this.field_78170_c[4]);
        this.terraslugWings[2] = new ModelRenderer(this, 20, 18);
        this.terraslugWings[2].addBox(-3.0F, 0.0F, (float)terraslugBoxLength[4][2] * -0.5F, 6, 5, terraslugBoxLength[1][2]);
    ...
    public void setRotationAngles(float f1, float f2, float f3, float f4, float f5, float f6, Entity f7)
    {
        for (int i = 0; i < this.terraslugBodyParts.length; ++i)
        {
            this.terraslugBodyParts[i].rotateAngleY = MathHelper.cos(f3 * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.05F * (float)(1 + Math.abs(i - 2));
            this.terraslugBodyParts[i].rotationPointX = MathHelper.sin(f3 * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.2F * (float)Math.abs(i - 2);
        }
